...
```java
// ✅ 正确：Redis存储授权状态
public void updateAuth(Long userId, Map<String, Boolean> authMap) {
    // 存储授权状态
    ...
    // 同步审计日志
    auditService.recordAuthLog(userId, authMap);
}
```
...
public @interface RequireAuth {
    String value();  // 授权项：location/phone等
}
...
public Result<List<StoreDTO>> getNearbyStores(@RequestParam Double lat, @RequestParam Double lng) {
...
| 数据类型 | 留存期限 | 清理方式 |
|----------|----------|----------|
| 浏览日志/搜索记录 | ≤90天 | 定时物理删除 |
| 订单/支付记录 | ≤3年 | 归档后删除 |
| 用户基本信息 | 注销后72小时 | 全链路删除 |
...
public void verifyCancelResult() {
    ...
    for (Long userId : pendingUserIds) {
        ...
        boolean esExist = esTemplate.exists(String.valueOf(userId), IndexCoordinates.of("order_index"));
...
public Result<String> verify2FA(
    ...
    if (!isValid) {
        // 记录失败次数，超过5次锁定
        int failCount = redisTemplate.opsForValue().increment("2fa:fail:" + username).intValue();
        if (failCount >= 5) {
